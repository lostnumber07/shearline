...
```
   ...
   stdio / HTTP         │  7 @mcp.tool() coroutines · CONUS bounds check  │
...
| --- | ---: | --- |
| `server.py` | ~505 | The 7 MCP tools, the CONUS gate on every entry, the per-tool **payload builders** (which call sources + derive and assemble the envelope), and the `get_threat_brief` fan-out. The only file that imports `FastMCP`. |
...
| `envelope.py` | ~26 | The `{data, interpretation, degraded, disclaimer}` builder and the exact disclaimer string. |
...
| **IEM** | `mesonet.agron.iastate.edu/api/1/nws/lsrs_by_point` | Time filter is silently ignored unless **both** `begints` and `endts` are sent. Magnitude semantics vary by type (hail inches, wind mph, tornado null). Case-varying unit strings. |
